...
trait WorldProtectTrait
{
    ...
    {
        if (!$this->getData()->exists('worldprotect')) {
            return [];
        }
        if (!isset($this->getData()->get('worldprotect')[$world])) {
            return [];
        }

        return $this->getData()->get('worldprotect')[$world] ?? [];
    }
    ...
    public function setWorldProtected(string $type, bool $value, string $worldName): void
    {
        ...
        $this->getData()->setNested("worldprotect.$worldName", $protects);
        $this->getData()->save();
    }
}
